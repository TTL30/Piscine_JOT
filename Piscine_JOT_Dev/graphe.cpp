...
//    *pmax=mespoid[mespoid.size()-1];
    ...
    if(dij==1)
    {
        ...
        possi[0].setpoiddij(djikstra(ma_matrice,INFINI,possi[0].getpoid(0)),poidselec);
        domine.push_back(possi[0]);
        ...
        for(graphe mesGr:possi)
        {
            ...
            dij_maillon=djikstra(ma_matrice,yref, mesGr.getpoid(0));
...
float graphe::djikstra(float** matrice_adjacence,float yref, float p)
{
    int *sommet_marques;
    float *long_min;
    int* pred;
